...
def build_policy():
    global policy
    W = np.zeros((3139), float)
    temp_W = np.zeros((3139), float)
    gamma = 0.1
    epsilon = 0.01
    converged = False
    ...
    while not converged:
            ...
            for j in range(9):
                ...
                for k in range(3139):
                    first_step_avg += transition_func[i][j][k] * \
                        get_reward(states[k])
                    pw_val += transition_func[i][j][k] * W[k]
                tot_val = first_step_avg + gamma * pw_val
                if tot_val > max_val:
                    max_val = tot_val
                    max_j = j
            temp_W[i] = max_val
        ...
        converged = True
        for i in range(3139):
            if abs(temp_W[i] - W[i]) > epsilon:
                print("Convergence Difference", abs(temp_W[i] - W[i]))
                converged = False
                break
        policy = temp_policy
        W = temp_W
    ...
    with open('vi_policy.json', 'w') as f:
